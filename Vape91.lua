...
run(function()
	...
	local function getTarget(origin, obj)
		...
		-- 检查目标是否在FOV内
		if ent and FOV and FOV.Value > 0 then
			local screenPoint, onScreen = gameCamera:WorldToViewportPoint(ent[targetPart].Position)
			if onScreen then
				-- 计算目标位置与屏幕中心的距离
				local center = vape.gui.AbsoluteSize / 2
				local mousePos = inputService:GetMouseLocation()
				local distance = (Vector2.new(screenPoint.X, screenPoint.Y) - center).Magnitude
				if distance > FOV.Value then
					return nil  -- 目标不在FOV内
				end
			else
				return nil  -- 目标不在屏幕上
			...
			if callback then
				...
				repeat
					...
					if AutoFire.Enabled then
						...
						-- 检查目标是否在FOV内
						if ent and FOV and FOV.Value > 0 then
							local screenPoint, onScreen = gameCamera:WorldToViewportPoint(ent.Head.Position)
							if onScreen then
								local center = vape.gui.AbsoluteSize / 2
								local distance = (Vector2.new(screenPoint.X, screenPoint.Y) - center).Magnitude
								if distance > FOV.Value then
	...
	SilentAim:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				CircleObject.Filled = CircleFilled.Enabled
				CircleObject.Color = Color3.fromHSV(CircleColor.Hue, CircleColor.Sat, CircleColor.Value)
				CircleObject.Position = vape.gui.AbsoluteSize / 2  -- 固定在屏幕中央
				CircleObject.Radius = FOV.Value
				CircleObject.NumSides = 100
				CircleObject.Transparency = 1 - CircleTransparency.Value
...
run(function()
	...
	local function revertClone()
		if not oldroot or not oldroot:IsDescendantOf(workspace) or not entitylib.isAlive the
			return false
		end
